...
test_compile_conf_crypt='../test_compile_conf_crypt.py'
python=python2

result='Ok'

for python in python2 python3 pypy; do
  if ! command -v "${python}" 2>&1 > /dev/null; then
    printf '\nTests for interpret %s skipped, because interpret is not found.\n' "${python}"
    continue
  fi
  printf '\nTests for interpret %s:\n' "${python}"

  echo
  echo "Unit tests:"
  if ! "${python}" "${test_compile_conf_crypt}"; then
    result='Failed'
  fi
  ...
  rm -f test1.conf
  USER=srbt VERSION=xena "${python}" "${compileConf}" \
      -k test1.key \
  ...
  encrypted=$("${python}" "${compileConfEncrypt}" --rsa -k test1.rsa.key <<< "hokus")
  echo
  ...
  encrypted=$("${python}" "${compileConfEncrypt}" --rsa -k test1.rsa.key <<< "$orig" | tr -d '\n')
  ...
  echo
  echo "Test zasifrovani promenne AES:"
  encrypted=$("${python}" "${compileConfEncrypt}" --aes -k test1.key <<< "hokus")
  echo
  printf '#compilable conf\nHokus: %s\n' "$encrypted" > test1a.conf.sample
  printf '# tento soubor vznikl generovanim pomoci compile_conf z test1a.conf.sample\nHokus: hokus\n' > test1a.conf.out
  rm -f test1a.conf
  ...
  if "${python}" "${compileConfEncrypt}" --rsa -k test1.rsa.key <<< "hokus'a"; then
  ...
  if "${python}" "${compileConf}" -K test1.rsa.key.priv production test1.conf.apost.sample test1.conf.apost; then
  ...
  if "${python}" "${compileConf}" -K test1.rsa.key.priv production test1.conf.quote.sample test1.conf.quote; then
